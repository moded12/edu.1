...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'], $_POST['class_id'])) {
  ...
  if ($name !== '' && $class_id > 0) {
    $stmt = $conn->prepare("INSERT INTO semesters (material_id, name) VALUES (?, ?)");
    $stmt->bind_param("is", $class_id, $name); // NOTE: column should be material_id
    $stmt->execute();
    header("Location: add_semester.php?application_id=" . intval($_POST['application_id']));
    exit;
  }
}
...
if (isset($_GET['delete'])) {
  ...
  header("Location: add_semester.php?application_id=" . intval($_GET['application_id']));
  exit;
}
...
$semesters = $conn->query("SELECT semesters.id, semesters.name, classes.name AS class_name
  FROM semesters
...
<div class="content">
  <h3 class="mb-4 text-primary">🕓 إضافة فصل دراسي</h3>
  ...
  <form method="POST" class="row g-3 mb-4">
    ...
    <div class="col-md-4">
      <input type="text" name="name" class="form-control" placeholder="اسم الفصل" required>
    </div>
    <div class="col-md-2">
  ...
  <!-- عرض الفصول -->
  <h5 class="mb-3">📋 الفصول الدراسية:</h5>
  <table class="table table-bordered bg-white">
    <thead class="table-light">
      <tr><th>#</th><th>الفصل</th><th>الصف</th><th>تحكم</th></tr>
    </thead>
    <tbody>
  ...
  <div class="modal fade" id="confirmDeleteModal" tabindex="-1" aria-labelledby="confirmDeleteModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body text-center">⚠️ هل تريد حذف هذا الفصل؟</div>
